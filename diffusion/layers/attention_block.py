...
class SelfAttentionBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        q = q.permute(0, *range(2, len(D)+2), 1).contiguous().reshape(B, np.prod(D), C)
        k = k.reshape(B, C, np.prod(D))
        v = v.permute(0, *range(2, len(D)+2), 1).contiguous().reshape(B, np.prod(D), C)

        w = torch.bmm(q, k) * (C**(-0.5))  # scaled channel attention matrix, QK^T / sqrt(d)
        w = torch.softmax(w, dim=-1)
        attention = torch.bmm(w, v)
        attention = attention.reshape(B, *D, C).permute(0, -1, *range(1, len(D)+1)).contiguous()
